...
ggplot(data = diamonds) +
  ...
  geom_jitter(width = 0, height = .2, alpha = .05) +
  ylab("")
````
...
Note that there is an `alpha` parameter in the `geom_boxplot()` function. What does it do?
There is an `alpha` parameter in the `geom_jitter()` function, too.
...
Try the same experiment to determine what the other parameters do.
:::
